...
from nlplingo.text.text_theory import Document, EntityRelation
...
class AceAnnotation(object):
    ...
    def process_ace_xmlfile(cls, doc, xml_file):
        ...
        cls.process_relations(doc, document_node)
        return doc
    ...
    @classmethod
    def process_relations(cls, doc, document_node):
        """
        :type doc: nlplingo.text.text_theory.Document
        :type document_node: xml.etree.ElementTree.Element
        """
        for relation_node in document_node.findall('relation'):
            relation_id = relation_node.attrib['ID']
            relation_type = relation_node.attrib['TYPE']
           
            if 'SUBTYPE' not in relation_node.attrib:	# these are TYPE=METONYMY, which are event-event relations
                continue

            relation_subtype = relation_node.attrib['SUBTYPE']

            for mention_node in relation_node.findall('relation_mention'):
                mention_id = mention_node.attrib['ID']

                roles = dict()
                for argument_mention_node in mention_node.findall('relation_mention_argument'):
                    ...
                    roles[arg_role] = arg_em

                assert 'Arg-1' in roles
                assert 'Arg-2' in roles
                relation = EntityRelation(relation_type + '.' + relation_subtype, roles['Arg-1'], roles['Arg-2'])
                doc.add_entity_entity_relation(relation)
