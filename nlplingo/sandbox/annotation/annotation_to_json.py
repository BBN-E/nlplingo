...
from nlplingo.nn.trigger_model import MaxPoolEmbeddedTriggerModel
from nlplingo.sandbox.common.parameters import Parameters
from nlplingo.sandbox.model import MaxPoolEmbeddedRoleModel
from nlplingo.sandbox.model import evaluate_f1
...
def get_predicted_positive_triggers(predictions, examples, none_class_index, event_domain):
    ...
    Also, use the predicted event_type for each such eventtrigger example

    :type predictions: numpy.nparray
    :type examples: list[nlplingo.tasks.event_trigger.EventTriggerExample]
    :type event_domain: tasks.event_domain.EventDomain
    """
    ...
    for i, index in enumerate(pred_arg_max):
        if index != none_class_index:
            eg = examples[i]
            """:type: tasks.event_trigger.EventTriggerExample"""
...
def argument_modeling(params, train_docs, test_docs, event_domain, word_embeddings, predicted_positive_triggers):
    ...
    :type event_domain: nlplingo.event.event_domain.EventDomain
    :type word_embeddings: nlplingo.embeddings.word_embeddings.WordEmbedding
    :type predicted_positive_triggers: list[nlplingo.tasks.event_trigger.EventTriggerExample]
    """
    generator = EventArgumentExampleGenerator(event_domain, params)
    ...
    with open('role.try.pickle', u'rb') as f:
        model = pickle.load(f)
        """:type: model.cnn.ExtractionModel"""
...
if __name__ == "__main__":
    ...
    # trigger_generator = EventTriggerExampleGenerator(event_domain)
    ...
    #
    #
    # argument_generator = EventArgumentExampleGenerator(event_domain)
