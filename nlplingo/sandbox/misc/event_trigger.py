...
class EventKeywordList(object):
    ...
    def print_statistics(self):
        # keywords which are associated with multiple tasks types
...
class EventTriggerGenerator(object):
    ...
    def __init__(self, event_domain, params, max_sentence_length, neighbor_distance, use_bio_index):
        ...
        #self.do_trigger_pcnn = params.get_boolean('eventtrigger.do_pcnn')
        ...
        # if an anchor has this span, we will always generate a eventtrigger example, tasks if we need to generate 'None' label
        if 'eventtrigger.spans_to_generate_examples' in params:
            self.spans_to_generate_examples = self._read_candidate_span_file(params['eventtrigger.spans_to_generate_examples'])
            """:type: defaultdict[str, set[IntPair]]"""
        else:
            self.spans_to_generate_examples = None

        self.restrict_none_examples_using_keywords = False
        if 'eventtrigger.restrict_none_examples_using_keywords' in params:
            self.restrict_none_examples_using_keywords = params['eventtrigger.restrict_none_examples_using_keywords']
            if self.restrict_none_examples_using_keywords:
                self.event_keyword_list = EventKeywordList(params['event_keywords'])

        if 'eventtrigger.do_not_tag_as_none.file' in params:
            self.do_not_tag_as_none_dict = defaultdict(set)
            with codecs.open(params['eventtrigger.do_not_tag_as_none.file'], 'r', encoding='utf-8') as f:
                for line in f:
    ...
    def _read_event_keywords(self, filepath):
        """ Read tasks keywords from a JSON file
    ...
    def generate(self, docs):
        ...
        for index, doc in enumerate(docs):
            #print('Generating eventtrigger candidates from doc {}'.format(doc.docid))
    ...
    def _generate_sentence(self, sentence, doc_entity_mentions):
        ...
        :rtype: list[nlplingo.tasks.event_trigger.EventTriggerExample]
        """
        ret = []
    ...
    def accept_tokens_as_candidate(self, tokens, event_type, entity_mentions, docid):
        """Whether to reject a token as eventtrigger candidate
        :type tokens: list[nlplingo.text.text_span.Token]
        ...
        #             break
        #     if not in_span:
        #         #print('Reject eventtrigger because it does not fall within trigger_candidate_span')
        #         return False
        ...
        if tokens[-1].pos_category() not in self.trigger_pos_category:
            self.statistics['Reject eventtrigger pos_category'] += 1
            if event_type != 'None':
                safeprint(u'Reject eventtrigger pos_category: pos={} text={} {}'.format(tokens[-1].pos_tag, tokens[-1].text, event_type))
                self.statistics['Reject eventtrigger pos_category TP'] += 1
            return False
        if "'" in tokens[-1].text:
            self.statistics['Reject eventtrigger \''] += 1
            if event_type != 'None':
                safeprint(u'Reject eventtrigger \': text={} {}'.format(tokens[-1].text, event_type))
                self.statistics['Reject eventtrigger \' TP'] += 1
            return False
        if re.search('\d', tokens[-1].text):     # there's a digit
            self.statistics['Reject eventtrigger digit'] += 1
            if event_type != 'None':
                safeprint(u'Reject eventtrigger digit: text={} {}'.format(tokens[-1].text, event_type))
                self.statistics['Reject eventtrigger digit TP'] += 1
            return False
        if len(tokens[-1].text) < 2:
            self.statistics['Reject eventtrigger len < 2'] += 1
            if event_type != 'None':
                safeprint(u'Reject eventtrigger len < 2: text={} {}'.format(tokens[-1].text, event_type))
                self.statistics['Reject eventtrigger len < 2 TP'] += 1
            return False
        if tokens[-1].text.lower() in self.negative_trigger_words:
            self.statistics['Reject eventtrigger negative-word'] += 1
            if event_type != 'None':
                safeprint(u'Reject eventtrigger negative-word: text={} {}'.format(tokens[-1].text.lower(), event_type))
                self.statistics['Reject eventtrigger negative-word TP'] += 1
            return False
        ...
        for em in entity_mentions:
            ...
            if em.head() == tokens[-1] and em.label != 'OTH':
                self.statistics['Reject eventtrigger overlap-EM'] += 1
                if event_type != 'None':
                    safeprint(u'Reject eventtrigger overlap-EM: docid={} text=({}) em-type={} {}'.format(docid, '_'.join(token.text.lower() for token in tokens), em.label, event_type))
                    self.statistics['Reject eventtrigger overlap-EM TP'] += 1
                return False
        ...
        if self.restrict_none_examples_using_keywords and event_type == 'None':
            ...
            if len(event_types) > 0:
                self.statistics['Reject eventtrigger as None since keyword for tasks-type'] += 1
                return False
    ...
    def _assign_word_vector_data_fb(anchor_token_indices, max_sent_length, example):
        ...
        :param example: nlplingo.tasks.event_trigger.EventTriggerExample
        :return: None
        """
    ...
    def _assign_ner_data(tokens, example):
        """
        :type tokens: list[nlplingo.text.text_span.Token]
        :type example: nlplingo.tasks.event_argument.EventTriggerExample
        """
