...
class MaxPoolEmbeddedWordPairModel(WordPairModel):
    ...
    def create_model(self):
        ...
        wordpair_input = Input(shape=(2, self.word_embeddings.shape[1]), name=u'wordpair_vector')
        ...
        wordpair_flattened = Flatten()(wordpair_input)

        # Input is vector of embedding indices representing the sentence
        #window_size = 2 * self.neighbor_dist + 1
        ####context_input = Input(shape=(self.sent_length, self.word_embeddings.shape[1]), name=u'word_vector')
        ...
        ####context_flattened = Flatten()(context_input)

        lex_input = Input(shape=(2 * self.neighbor_dist + 1, self.word_embeddings.shape[1]), name=u'lex_vector')
        lex_flattened = Flatten()(lex_input)
        ...
        if self.use_dropout:
            wp_in = Dropout(self.dropout)(wordpair_flattened)
            lex_in = Dropout(self.dropout)(lex_flattened)
        else:
            wp_in = wordpair_flattened
            lex_in = lex_flattened

        wp_hidden = Dense(self.number_of_hidden_nodes[0], activation=u'sigmoid')(wp_in)
        lex_hidden = Dense(self.number_of_hidden_nodes[0], activation=u'sigmoid')(lex_in)

        out = keras.layers.multiply([wp_hidden, lex_hidden])
        out_final = Dense(1, activation=u'sigmoid')(out)

        #keras_wordpair_model = Model(input=[context_input, lex_input, wordpair_input], output=[out_final])
        keras_wordpair_model = Model(input=[lex_input, wordpair_input], output=[out_final])
