from nlplingo.oregon.event_models.uoregon.models.pipeline._01.modules.argument_model import ArgumentModel
from nlplingo.oregon.event_models.uoregon.models.pipeline._01.iterators import ArgumentIterator, get_arguments
...
class ArgumentTrainer:
    def __init__(self, opt, eval_mode=False):
        ...
        if not eval_mode:
            self.train_iterator = ArgumentIterator(
                xlmr_model=self.model.xlmr_embedding,
                # data_path=os.path.join(MODEL_DIR,			# <==
                data_path=os.path.join(self.opt['datapoint_dir'],	# ==>
                                       opt['data_map']['argument']['train'].format(get_data_dir(self.opt['data'])))
            )
            print('Argument model : Training data: {}'.format(self.train_iterator.num_examples))

            self.dev_iterator = ArgumentIterator(
                xlmr_model=self.model.xlmr_embedding,
                # data_path=os.path.join(MODEL_DIR,			# <==
                data_path=os.path.join(self.opt['datapoint_dir'],	# ==>
                                       opt['data_map']['argument']['dev'].format(get_data_dir(self.opt['data']))),
                is_eval_data=True
            )
    ...
    def _print_args(self):
        ...
        logger.info('-' * 100)
        logger.info('> trainable params: Argument model')
        for name, param in self.model.named_parameters():
    ...
    def train(self):
        ...
        logger.info('Start training: Argument model')
    ...
    def save_model(self, epoch):
        ...
        try:
            ...
            write_json(opt, write_path=os.path.join('checkpoints',
                                                    'argument.epoch-{}.config'.format(epoch)))
        except BaseException:
