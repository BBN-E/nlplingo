...
class TriggerGenerator(object):
    ...
    def encode_trigger_example(self, example, doc, label_map):
        ...
        head_ids = []
        dep_rel_ids = []
        for token in example.sentence.tokens:
            if len(token.dep_relations) > 0:
                head_id = token.dep_relations[0].connecting_token_index
                dep_rel = deprel_map.get(token.dep_relations[0].dep_name, deprel_map['[UNK]'])
            else:
                head_id = 0
                dep_rel = deprel_map['[UNK]']
            head_ids.append(head_id)
            dep_rel_ids.append(dep_rel)
...
class PipelineGenerator(object):
    ...
    """
    def __init__(self, xlmr_model, examples, docs, biw2v_map):
