#from python.clever.event_models.uoregon.models.pipeline._01.modules.ED_model import EDModel
#from python.clever.event_models.uoregon.models.pipeline._01.iterators import EDIterator
...
from nlplingo.oregon.nlplingo.tasks.sequence.ED_model_hf import EDModelHF
from nlplingo.oregon.nlplingo.tasks.sequence.generator_hf import TriggerGenerator
from nlplingo.oregon.nlplingo.tasks.sequence.tokenizer import Tokenizer


class EDTrainer(object):
    def __init__(self, opt, logger, biw2v_map, eval_mode=False, train_examples=None, train_docs=None, dev_examples=None, dev_docs=None, label_map=None):
        ...
        self.opt = opt
        self.logger = logger
        self.model = EDModelHF(opt, label_map)
        self.model.to(self.opt['device'])

        self.tokenizer = Tokenizer(self.opt)
        ...
        if not eval_mode:
            # the following encodes the data (i.e. generates features for each example), and creates batches
            self.train_iterator = TriggerGenerator(opt, self.model.xlmr_embedding, self.tokenizer, train_examples, train_docs, label_map, biw2v_map, is_eval_data=False)
            print('ED model : Training data: {}'.format(self.train_iterator.num_examples))

            self.dev_iterator = TriggerGenerator(opt, self.model.xlmr_embedding, self.tokenizer, dev_examples, dev_docs, label_map, biw2v_map, is_eval_data=True)
    ...
    def _print_args(self):
        print('========== ED_trainer_hf _print_args ============')
        ...
        self.logger.info('> trainable params: ED model')
        for name, param in self.model.named_parameters():
    ...
    def record_scores(self, preds, labels, lang_weights):
        def compute_TP_FP_FN(ps, ls):
            ls2 = torch.reshape(ls, (-1,))
            ps2 = torch.reshape(ps, (-1,))

            num_true = torch.sum(ls2 != self.label_map['O'])
            num_predict = torch.sum(ps2 != self.label_map['O'])
            c = 0
            for i, j in zip(ls2, ps2):
                if i == j and i != self.label_map['O']:
                    c += 1
            TP_FN = num_true
            TP_FP = num_predict
            TP = c
    ...
    def eval(self, epoch):
        ...
        for b_id, batch in enumerate(self.dev_iterator):    # this calls TriggerGenerator.__getitem__
            batch = move_to_device(batch, self.opt['device'])
    ...
    def load_saved_model(self):
        model_file = os.path.join(self.opt['trigger_model_dir'], 'ED.model')
    ...
    def save_model(self, epoch):
        ...
        try:
            output_dir = os.path.join(self.opt['trigger_model_dir'], 'epoch-%d' % (epoch))
            ensure_dir(output_dir)

            ckpt_fpath = os.path.join(output_dir, 'ED.model')
            # ckpt_fpath = os.path.join('checkpoints', 'ED-model.seed-{}.epoch-{}.saved'.format(self.opt['seed'], epoch))
            torch.save(params, ckpt_fpath)
            print('... to: {}'.format(ckpt_fpath))

            opt['trigger_model_dir'] = output_dir
            write_json(opt, write_path=os.path.join(output_dir, 'ED.config'))
            # write_json(opt, write_path=os.path.join('checkpoints', 'ED.epoch-{}.config'.format(epoch)))
        except BaseException:
